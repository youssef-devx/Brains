...
module.exports = function conjugate(tense, verb, pronoun, form) {
  if(tense.includes("past")) {
    return pastTenses(tense, verb, pronoun, form)
  } else if(tense.includes("present")) {
    return presentTenses(tense, verb, pronoun, form)
  } else if (tense.includes("future")) {
    return futureTenses(tense, verb, pronoun, form)
  }
}

function pastTenses(tense, verb, pronoun, form) {
  const isI = pronoun === "I"
  const isThirdSingular = thirdSingularPronouns.includes(pronoun)
  const isNegative = form === "negative"
  const wasOrWere = isI || isThirdSingular ? `${isNegative ? "was not" : "was"}` : `${isNegative ? "were not" : "were"}`

  if(tense === "simple past"){
    if(verb === "be") return wasOrWere
    if(!Object.keys(irrVerbs).includes(verb)){
      return `${isNegative ? `did not ${verb}ed` : `${verb}ed`}`
    } else {
      if(irrVerbs[verb].length === 4) {
        return `${isNegative ? `did not ${irrVerbs[verb][0]}` : irrVerbs[verb][0]} or ${isNegative ? `did not ${irrVerbs[verb][1]}` : irrVerbs[verb][1]}`
        // return `${isNegative ? `did not ${irrVerbs[verb][0]}` : `${irrVerbs[verb][0]}`} or ${isNegative ? `did not ${irrVerbs[verb][1]}` : `${irrVerbs[verb][1]}`}`
      } else {
        return isNegative ? `did not ${irrVerbs[verb][0]}` : irrVerbs[verb][0]
      }
    }
  }
  ...
  if(tense ===  "past perfect") {
    if(verb === "be") return isNegative ? "had not been" : "had been"
    if(!Object.keys(irrVerbs).includes(verb)){
      return isNegative ? `had not ${verb}ed` : `had ${verb}ed`
    } else {
      if(irrVerbs[verb].length === 4) {
        return `${isNegative ? "had not" : "had"} ${irrVerbs[verb][2]} or ${isNegative ? "had not" : "had"} ${irrVerbs[verb][3]}`
      } else {
        return isNegative ? `had not ${irrVerbs[verb][1]}` : `had ${irrVerbs[verb][1]}`
      }
    }
  }
  if(tense === "past perfect progressive") {
    if(verb === "be") return isNegative ? "had not been" : "had been"
    if(isCVC(verb)) {
      return `${isNegative ? "had not" : "had"} been ${verb}${verb[verb.length-1]}ing`
    } else if(/[aeuio][aeuio]/.test(verb.split("").splice(verb.length-2))) {
      return `${isNegative ? "had not" : "had"} been ${verb.splice(0, verb.length-1)}ing`
    } else {
      return `${isNegative ? "had not" : "had"} been ${verb}ing`
    }
  }
}

function presentTenses(tense, verb, pronoun, form) {
  const isI = pronoun === "I"
  const isThirdSingular = thirdSingularPronouns.includes(pronoun)
...
function futureTenses(tense, verb, pronoun, form) {
